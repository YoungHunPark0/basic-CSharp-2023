...
namespace wf09_dbhandling
{
    public partial class FrmMain : Form
    {
        ...
        private void FrmMain_Load(object sender, EventArgs e)
        {
            ...
            // 연결문자열 없이 객체 생성X // 첫번째방법
            SqlConnection conn = new SqlConnection(connectionString); // connectionString 필수로 들어가야함
            //conn.ConnectionString = connectionString; // 두번째 방법

            // 3. 객체를 통해서 DB 연결
            conn.Open();

            // 4. DB처리를 위한 쿼리 작성 // @ == 리터럴문자열 여러줄 문자열을 사용할 수 있게함
            // ssms에서 테이블 -> 새쿼리편집기창 에서 따옴
            // 5. SqlDataAdapter 생성
            ...
            SqlDataAdapter adapter = new SqlDataAdapter(selQuery, conn); // 쿼리를 어뎁터에 보냄
